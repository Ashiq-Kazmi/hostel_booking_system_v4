...
def create_app():
    ...
    app.config.update(
        ...
        SQLALCHEMY_DATABASE_URI=os.environ.get("DATABASE_URL", "sqlite:///hostel_booking.db").replace("postgres://", "postgresql+psycopg://", 1),
    ...
    def hostels():
        ...
        if q:
            query = query.filter(db.or_(Hostel.name.ilike(f"%{q}%"), Hostel.location.ilike(f"%{q}%")))
        if room_type:
    ...
    def hostel_detail(hostel_id):
        ...
        if hostel.listing_status != "active" and session.get("role") != "admin" and session.get("user_id") != hostel.hostel_admin_id:
    ...
    def student_register():
        if request.method == "POST":
            ...
            try:
                db.session.add(Student(name=name, email=email, password=generate_password_hash(password), phone=phone, university=university))
                db.session.commit()
    ...
    def student_login():
        if request.method == "POST":
            ...
            if user and check_password_hash(user.password, request.form.get("password", "")):
                session.clear(); session.update(user_id=user.id, role="student")
    ...
    def hostel_admin_register():
        if request.method == "POST":
            ...
            try:
                db.session.add(HostelAdmin(full_name=name, email=email, password=generate_password_hash(password), phone=phone))
                db.session.commit()
                flash("Hostel owner account created. You can now sign in.", "success")
    ...
    def hostel_admin_login():
        if request.method == "POST":
            ...
            if user and check_password_hash(user.password, request.form.get("password", "")):
                session.clear(); session.update(user_id=user.id, role="hostel_admin")
    ...
    def admin_login():
        if request.method == "POST":
            ...
            if user and check_password_hash(user.password, request.form.get("password", "")):
                session.clear(); session.update(user_id=user.id, role="admin")
    ...
    def booking(hostel_id):
        hostel = db.get_or_404(Hostel, hostel_id)
        if hostel.listing_status != "active":
            flash("This hostel is not available for booking.", "danger"); return redirect(url_for("hostels"))
        if hostel.seats <= 0:
            flash("No seats are currently available.", "danger"); return redirect(url_for("hostel_detail", hostel_id=hostel.id))
        ...
        if existing:
            flash("You already have an active booking request for this hostel.", "warning"); return redirect(url_for("my_bookings"))
        hostel.seats -= 1
        db.session.add(Booking(student_id=session["user_id"], hostel_id=hostel.id))
        db.session.commit()
    ...
    @login_required("student")
    def my_bookings():
        bookings = Booking.query.filter_by(student_id=session["user_id"]).order_by(Booking.booking_date.desc()).all()
    ...
    def review(hostel_id):
        ...
        if rating not in range(1, 6):
            ...
            db.session.add(Review(student_id=session["user_id"], hostel_id=hostel_id, rating=rating, comment=request.form.get("comment", "").strip()))
            db.session.commit(); flash("Review submitted.", "success")
        return redirect(url_for("hostel_detail", hostel_id=hostel_id))

    @app.route("/hostel-admin/dashboard")
    @login_required("hostel_admin")
    ...
    def add_hostel():
        if request.method == "POST":
            ...
            db.session.add(Hostel(hostel_admin_id=session["user_id"], name=request.form.get("name", "").strip(), location=request.form.get("location", "").strip(), price=price, seats=seats, room_type=request.form.get("room_type", "").strip(), distance_km=distance, description=request.form.get("description", "").strip(), image_url=request.form.get("image_url", "").strip(), map_link=request.form.get("map_link", "").strip()))
            db.session.commit(); flash("Hostel submitted for admin approval.", "success"); return redirect(url_for("hostel_admin_dashboard"))
    ...
    def owner_booking_action(booking_id, action):
        ...
        if booking.hostel.hostel_admin_id != session["user_id"]: return redirect(url_for("hostel_admin_dashboard"))
        if action == "approve" and booking.status == "pending": booking.status = "approved"
        elif action == "reject" and booking.status in ("pending", "approved"):
            booking.status = "rejected"; booking.hostel.seats += 1
    ...
    def payment(hostel_id):
        ...
        if amount <= 0: flash("Enter a valid payment amount.", "danger")
        else:
            db.session.add(Payment(hostel_id=hostel.id, hostel_admin_id=session["user_id"], amount=amount, payment_method=request.form.get("payment_method", "").strip(), transaction_ref=request.form.get("transaction_ref", "").strip()))
            db.session.commit(); flash("Payment submitted for verification.", "success")
        return redirect(url_for("hostel_admin_dashboard"))
    ...
    def payment_action(payment_id, action):
        ...
        if action == "approve":
            payment.verification_status = "approved"; payment.hostel.payment_status = "paid"; payment.hostel.listing_status = "active"
        elif action == "reject": payment.verification_status = "rejected"
    ...
    def hostel_action(hostel_id, action):
        hostel = db.get_or_404(Hostel, hostel_id)
        if action == "approve": hostel.listing_status = "active"
        elif action == "disable": hostel.listing_status = "inactive"
        else: flash("Invalid hostel action.", "warning"); return redirect(url_for("admin_dashboard"))
    ...
    def decorator(view):
        ...
        def wrapped(*args, **kwargs):
            if session.get("role") != role:
                ...
                target = {"student": "student_login", "hostel_admin": "hostel_admin_login", "admin": "admin_login"}[role]
...
class Hostel(db.Model):
    ...
    distance_km = db.Column(db.Numeric(8, 2), default=0, nullable=False)
    description = db.Column(db.Text)
    image_url = db.Column(db.String(500))
    map_link = db.Column(db.String(500))
    listing_status = db.Column(db.String(20), default="inactive", nullable=False)
    ...
    __table_args__ = (CheckConstraint("price >= 0", name="ck_hostel_price_nonnegative"), CheckConstraint("seats >= 0", name="ck_hostel_seats_nonnegative"),)
...
class Review(db.Model):
    ...
    __table_args__ = (CheckConstraint("rating >= 1 AND rating <= 5", name="ck_review_rating"), UniqueConstraint("student_id", "hostel_id", name="uq_student_hostel_review"),)
